...
Edges: {edges}
    ...
    def word_from_trajectory(self, trajectory):
        ...
        for state, next_state in zip(trajectory, trajectory[1:]):
            # Ensure 'input' exists in the edge data before accessing it
            if 'input' in self.g[state][next_state]:
                symbol = next(iter(self.g[state][next_state]['input']))
            else:
                # Handle cases where 'input' is missing (you can decide how to handle this case)
                continue
            
            # Convert symbol to the set of propositions
        ...
        for state in self.g:
            for symbol in self.alphabet:
                next_states = [v for v in self.g[state]
                               if 'input' in self.g[state][v] and symbol in self.g[state][v]['input']]
